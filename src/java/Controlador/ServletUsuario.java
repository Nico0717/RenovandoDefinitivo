...
public class ServletUsuario extends HttpServlet {
     ...
     */
     protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String Contraseña = request.getParameter("textclave");

        BeanUsuario BUser = new BeanUsuario(Usuario, Contraseña);
        ...
        switch (opcion) {
            ...
            case 2: //VALIDAR USUARIO 
                if(DUser.validar(Usuario, Contraseña)){
                
                request.getRequestDispatcher("menu.jsp").forward(request, response);
        } else {
                request.setAttribute("error", "<script> alert('El Usuario y/o contraseña erroneos')</script>");
                request.getRequestDispatcher("index.jsp").forward(request, response);
